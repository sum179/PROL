...
class Imagenet_A(torch.utils.data.Dataset):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False, allow_empty=True):        
        self.root = os.path.expanduser(root)
        self.transform = transform
        self.target_transform=target_transform
        self.train = train
        ...
        print(fpath)
        # self.data = datasets.ImageFolder(fpath, transform=transform, allow_empty=True)
        self.data = datasets.ImageFolder(fpath, transform=transform)
...
#         self.url = 'https://openaipublic.azureedge.net/clip/data/country211.tgz'
#         self.filename = 'country211.tgz'

#         self.fpath = os.path.join(root, 'country211')
#         if not os.path.isfile(self.fpath):
#             if not download:
#                raise RuntimeError('Dataset not found. You can use download=True to download it')
#             else:
...
#         if not os.path.exists(os.path.join(root, 'country211')):
#             import tarfile
#             tar_ref = tarfile.open(os.path.join(root, self.filename), 'r')
#             tar_ref.extractall(root)
#             tar_ref.close()
