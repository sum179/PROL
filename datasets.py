...
def build_continual_dataloader(args):
    ...
    print("Check dataset name: ",args.dataset)
...
def get_dataset(dataset, transform_train, transform_val, args,):
    # print("Check get dataset: ",dataset)
    if dataset == 'CIFAR100':
        ...
        # dataset_train = Country211(args.data_path, train=True, download=True, transform=transform_train).data
        # dataset_val = Country211(args.data_path, train=False, download=True, transform=transform_val).data
        # dataset_feat_train = Country211(args.data_path, train=True, download=True, transform=transform_val).data
        dataset_train = datasets.Country211(args.data_path, split='train', download=True, transform=transform_train)
        dataset_val = datasets.Country211(args.data_path, split='test', download=True, transform=transform_val)
        dataset_feat_train = datasets.Country211(args.data_path,  split='train', download=True, transform=transform_val)

    elif dataset == 'FGVCA':
        # dataset_train = Country211(args.data_path, train=True, download=True, transform=transform_train).data
        # dataset_val = Country211(args.data_path, train=False, download=True, transform=transform_val).data
        # dataset_feat_train = Country211(args.data_path, train=True, download=True, transform=transform_val).data
        dataset_train = datasets.FGVCAircraft(args.data_path, split='train', download=True, transform=transform_train)
        dataset_val = datasets.FGVCAircraft(args.data_path, split='test', download=True, transform=transform_val)
        dataset_feat_train = datasets.FGVCAircraft(args.data_path,  split='train', download=True, transform=transform_val)
...
def split_single_dataset(dataset_train, dataset_val, args, dataset_feat_train=None,):
    ...
    labels = [i for i in range(classes_per_task*args.num_tasks)]
    ...
    for _ in range(args.num_tasks):
        ...
        mask.append(scope)
        # print(dataset_train._labels)

        if "FGVCA" in args.dataset:
            for k in range(len(dataset_train._labels)):
                if int(dataset_train._labels[k]) in scope:
                    train_split_indices.append(k)
                    feat_split_indices.append(k)
                    
            for h in range(len(dataset_val._labels)):
                if int(dataset_val._labels[h]) in scope:
